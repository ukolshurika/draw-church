...
> **AI-агент** — все инструменты проекта (парсинг, нормализация, слияние)
> работают через [OpenCode](https://opencode.ai) с бесплатными моделями
> (от DeepSeek Flash и выше). Достаточно [скачать установщик](https://opencode.ai/download)
> или открыть [opencode.ai/docs](https://opencode.ai/docs) для инструкций.
>
> **Рекомендуется Git.** Все изменения данных (слияния, нормализация) записываются
> в файлы — используйте систему контроля версий, чтобы в случае ошибки агента
...
- **manual-merging** — `.agents/skills/manual-merging/SKILL.md` — ручное объединение дубликатов персон

Пример промта для ручного слияния:
